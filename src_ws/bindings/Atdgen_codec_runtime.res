@bs.new external _unsafeCreateUninitializedArray: int => array<'a> = "Array"
let _stringify = j => DebugHelper.debugClean(j, 1)
...
module Json_adapter = {
  ...
  module Kind_field = Type_field.Make({
...
module Encode = {
  ...
  let field = (~default=?, encode, ~name, data) => F(
    @implicit_arity Required({name: name, data: data, encode: encode}, default),
  )

  let field_o = (~default=?, encode, ~name, data) => F(
    @implicit_arity Optional({name: name, data: data, encode: encode}, default),
  )

  let obj = fields => List.fold_left((acc, F(f)) =>
      switch f {
      | @implicit_arity Required({name, data, encode}, None) => list[(name, encode(data)), ...acc]
...
module Decode = {
  ...
  let with_segment = (segment, f, json) =>
    ...
    | DecodeError(msg) =>
      Js.Console.trace()
      Js.log(msg)
      raise(@implicit_arity DecodeErrorPath(list[segment], msg))
    | @implicit_arity DecodeErrorPath(path, msg) =>
      raise(@implicit_arity DecodeErrorPath(list[segment, ...path], msg))
    }
  ...
  let unit = j =>
    if (Obj.magic(j): Js.null<'a>) === Js.null {
      ...
      raise(DecodeError(sprintf("Expected null, got %s", Js.Json.stringify(j))))
    }
  ...
  let fieldDefault = (s, default, f) => fieldOptional(s, f) |> map(x =>
      switch x {
      | None => default
      | Some(s) => s
      }
    ...
    | #Constr(s, args) =>
      with_segment(
        ...
        () =>
          ...
          | exception Not_found => \"@@"(raise, DecodeError(sprintf("unknown constructor %S", s)))
          | #Single(_) =>
            \"@@"(raise, DecodeError(sprintf("constructor %S doesn't expect arguments", s)))
    ...
    either(
      x =>
        if string(x) == "None" {
          ...
          raise(DecodeError(sprintf("Expected None, got %s", Js.Json.stringify(x))))
        },
      x =>
  ...
  let adapter = (normalize: Json.t => Json.t, reader: t<'a>, json) => reader(normalize(json))
}
