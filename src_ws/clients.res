...
let createFile = (~client_id, filePath) => {
    ...
    Hashtbl.replace(map, client_id, {
        ...client,
        files: client.files->Belt.Array.concat([
            (fileId, filePath)
        ])
    });
    fileId
...
let setFileContents = (~client_id, ~file_id, contents) => {
    ...
    let client = Hashtbl.find(map, client_id);
    let filePath = client.files->Belt.Array.keep(((f_id,_)) => f_id === file_id)->Belt.Array.map(snd)->Belt.Array.getExn(0);
