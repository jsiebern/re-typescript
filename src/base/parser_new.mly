...
%public let property_name :=
  ...
  | s = literal_string; { let (s, _, pi) = s in Ts.PString({ Ts.pi; item = s }) }
  | n = literal_number; { let (n, pi) = n in Ts.PNumber({ Ts.pi; item = n }) }
...
%public let declaration_type :=
  | TYPE; i = identifier_name; tp = type_parameters?; EQUALS; te = type_; { Ts.Type({t_ident = i; t_parameters = tp; t_type = te}) }
...
%public let type_object :=
  | t = delimited(LCURLY, type_member_list?, RCURLY); { Ts.Object(t) }
...
%public let parameter_required :=
  | acm = accessibility_modifier?; ib = identifier_or_binding; ta = type_annotation?; { { Ts.p_optional = false; p_ibinding = ib; p_type_annotation = ta; p_accessibility = acm } }
  | bi = binding_identifier; sl = literal_string; { { Ts.p_optional = false; p_ibinding = bi; p_type_annotation = Some(Ts.Union(sl, None)); p_accessibility = None } }
...
%public let parameter_optional :=
  | acm = accessibility_modifier?; ib = identifier_or_binding; QMARK; ta = type_annotation?; { { Ts.p_optional = true; p_ibinding = ib; p_type_annotation = ta; p_accessibility = acm } }
  | bi = binding_identifier; QMARK; sl = literal_string; { { Ts.p_optional = true; p_ibinding = bi; p_type_annotation = Some(Ts.Union(sl, None)); p_accessibility = None } }
...
%public let type_reference :=
  | tn = identifier_path; ta = type_arguments?; { Ts.TypeReference(tn, ta) }
...
%public let literal_string :=
  | s = STRING; { let (s,_,pi) = s in Ts.StringLiteral({ Ts.pi; item = s }) }

%public let literal_number :=
  | n = NUMBER; { let (n, pi) = n in Ts.NumberLiteral( Ts.pi; item = n ) }
