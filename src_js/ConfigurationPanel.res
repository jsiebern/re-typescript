...
module Tooltip = {
  ...
  let make = (~tooltip) => {
...

    {
      ...
      | Some(tt) =>
        <Popup
          trigger={<Button compact=true circular=true icon="help" size=#mini color=#blue />}
  ...
  let make = (~tooltip=?, ~disabled=?, ~label, ~selector) => {
    ...
    <FormField inline=true>
      <Grid columns=Grid.Columns.equal>
        <GridColumn className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
...
module ConfigSelect = {
  ...
  let make = (
    ...
    let id = useId()
    let (value, setValue) = Recoil.useRecoilState(selector)
    let values = React.useMemo0(() => values->Belt.Array.mapU((. (v, l)) => (v, l, makeId())))
      ...
      <Grid columns=Grid.Columns.equal>
        ...
        <GridColumn className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
          <FormSelect
            ...
            inline=true
            ?disabled
            options={values->Belt.Array.mapU((. (v, l, _)) => {
...
module ConfigForm = {
  ...
  let index_state = Recoil.atom({key: "index_state", default: IntSet.fromArray([5])})
  module AsAccordion = {
    @react.component
    let make = (~label, ~index, ~children) => {
      let (state, setState) = Recoil.useRecoilState(index_state)
      let active = React.useMemo1(() => state->IntSet.has(index), [state])
      let handleClick = (_, _) =>
        setState(state =>
          state->IntSet.has(index) ? state->IntSet.remove(index) : state->IntSet.add(index)
        )
      ...
      <>
        <AccordionTitle index=0 active onClick=handleClick>
          <Icon name="dropdown" /> {label->React.string}
        </AccordionTitle>
  ...
  let make = () => {
    ...
    let (bs_number_value, set_bs_number_value) = Recoil.useRecoilState(State.bs_number_value)
            ...
            <Accordion styled=true fluid=true>
              <AsAccordion index=5 label="Generator Target">
                <ConfigSelect
                  ...
                  to_string={v => v->Config_bs.write_output_type->Js.Json.stringify}
                  from_string={v => v->Js.Json.parseExn->Config_bs.read_output_type}
                  label="Output Mode"
                  selector=State.output_type
                />
                {switch output_type {
                | Native => React.null
                | Bucklescript | BucklescriptBindings => <>
                    <ConfigSelect
                      values=[
                        (#Variant, "Variant"),
                      ...
                      to_string={v => v->Config_bs.write_string_variant_mode->Js.Json.stringify}
                      from_string={v => v->Js.Json.parseExn->Config_bs.read_string_variant_mode}
                    ...
                    <ConfigSelect
                      ...
                      to_string={v => v->Config_bs.write_mixed_variant_mode->Js.Json.stringify}
                      from_string={v => v->Js.Json.parseExn->Config_bs.read_mixed_variant_mode}
                    ...
                    <FormField inline=true>
                      <Grid columns=Grid.Columns.equal>
                        <GridColumn
                          ...
                          <label>
                            <Tooltip tooltip=None /> {"Custom Pre/Suffix"->React.string}
                          </label>
                        </GridColumn>
                        <GridColumn
                          className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
                          <Checkbox
                            ...
                            onChange={(_, _) => set_bs_number_value(v =>
                                switch v {
                    ...
                    | Some({prefix, suffix}) => <>
                        <FormField inline=true>
                          <Grid columns=Grid.Columns.equal>
                            <GridColumn
                              className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
                              <label> <Tooltip tooltip=None /> {"Prefix"->React.string} </label>
                            </GridColumn>
                            <GridColumn
                              className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
                              <input value=prefix onChange={e => set_bs_number_value(_ => Some({
                                    prefix: ReactEvent.Form.target(e)["value"],
                                    suffix: suffix,
                                  }))} />
                            </GridColumn>
                          </Grid>
                        ...
                        <FormField inline=true>
                          <Grid columns=Grid.Columns.equal>
                            <GridColumn
                              className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
                              <label> <Tooltip tooltip=None /> {"Suffix"->React.string} </label>
                            </GridColumn>
                            <GridColumn
                              className=%css(j`padding: $zero; align-items: $cen; display: $ifl;`)>
                              <input value=suffix onChange={e => set_bs_number_value(_ => Some({
                                    prefix: prefix,
                                    suffix: ReactEvent.Form.target(e)["value"],
                                  }))} />
                            </GridColumn>
                          </Grid>
                        </FormField>
              ...
              <AsAccordion index=0 label="Interface Extension">
                ...
                />
                <ConfigSwitch
                  label="Omit empty interfaces" selector=State.omit_extended_unreferenced_records
                />
              </AsAccordion>
              <AsAccordion index=1 label="Base Types">
                <ConfigSelect
                  values=[(Config_t.Float, "Float"), (Int, "Int"), (Unboxed, "Unboxed")]
              ...
              <AsAccordion index=2 label="Intersection generation">
                {
                  let values = [(Config_bs.Merge, "Merge"), (Tuple, "Tuple"), (Ignore, "Ignore")]
                  let from_string = v => v->Js.Json.parseExn->Config_bs.read_intersection_def
                  let to_string = v => v->Config_bs.write_intersection_def->Js.Json.stringify
                  <>
                    <ConfigSelect
                      values
                      to_string
              ...
              <AsAccordion index=3 label="Parser / Serializer">
                <ConfigSwitch
                  label="Generate parsers" selector=State.generate_parser disabled=true
                />
                <ConfigSwitch
                  label="Generate serializers" selector=State.generate_serializer disabled=true
                />
              </AsAccordion>
            </Accordion>
          </Form>
