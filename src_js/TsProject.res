...
let selectedSource = Recoil.selectorWithWrite({
  ...
  set: ({get, set}, value) => {
    ...
    set(selected, {...get(selected), action})
    | _ => ()
    }
  },
})
  ...
  get: ({get}) => {
    ...
    Js.Promise.make((~resolve, ~reject) => {
      ...
      switch (sourceFile) {
        ...
        | Some(sourceFile) => sourceFile->SourceFile.refreshFromFileSystemSync
        ...
        let parsed = (sourceFile->SourceFile.getFilePath, parsed->Typescript_bs.write_node->Js.Json.stringify)
    ...
    |> Js.Promise.then_(((file_path, content)) => {
      ...
      WorkerBridge.queryWorker(Parse({content, language, file_path, config}), (. result) => {
        switch result {
