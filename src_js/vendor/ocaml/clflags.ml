...
and open_modules: string list ref = ref []               (* -open *)
...
let pic_code = ref (match "architecture" with (* -fPIC *)
                     | "amd64" -> true
                     | _       -> false)
...
let default_inline_threshold = if false then 10. else 10. /. 8.
...
let all_passes: string list ref = ref []
...
let parse_color_setting = function
  ...
  | _ -> None
let color: Re_typescript.Misc.Color.setting option ref = ref None ;; (* -color *)
